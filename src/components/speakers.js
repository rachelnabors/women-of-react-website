import React from "react"
import cassidy from "../images/cassidy.jpg"
import yuraima from "../images/yuraima.jpg"
import neha from "../images/neha.jpg"
import taley from "../images/taley.jpg"
import maggie from "../images/maggie.jpg"
import anushree from "../images/anushree.jpg"
import eve from "../images/eve.jpg"
import av from "../images/av.jpg"
import carolyn from "../images/carolyn.jpg"
import olena from "../images/olena.jpg"
import jen from "../images/jen.jpg"

export default () => (
  <>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/cassidoo"
      >
        <img src={cassidy} alt="" />
        <p>Cassidy Williams</p>
      </a>
      Master of Ceremonies
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/yurm04"
      >
        <img src={yuraima} alt="" />
        <p>Yuraima Estevez</p>
      </a>
      TL;DR Accessible Components
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/hellonehha"
      >
        <img src={neha} alt="" />
        <p>Neha Sharma</p>
      </a>
      Design Systems
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/TaleyaMirza"
      >
        <img src={taley} alt="" />
        <p>Taley'a Mirza </p>
      </a>
      Level up your Design System with styled-system
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/Mappletons"
      >
        <img src={maggie} alt="" />
        <p>Maggie Appleton</p>
      </a>
      Drawing the Invisible
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/imAnushree"
      >
        <img src={anushree} alt="" />
        <p>Anushree Subramani</p>
      </a>
      React Dev Tools
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/eveporcello"
      >
        <img src={eve} alt="" />
        <p>Eve Porcello</p>
      </a>
      Finding the Fun: Using <abbr title="Test Drive Development">TDD</abbr>{" "}
      with React
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/adriannavaldivi"
      >
        <img src={av} alt="" />
        <p>Adrianna Valdivi </p>
      </a>
      Preparing for React tech interviews
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/carolstran"
      >
        <img src={carolyn} alt="" />
        <p>Carolyn Stransky</p>
      </a>
      The Most Important UI: You
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/frontendgirl"
      >
        <img src={olena} alt="" />
        <p>Olena Sovyn</p>
      </a>
      Testing, tEstIng, TeStInG
    </li>
    <li>
      <a
        rel="noopener noreferrer"
        target="_blank"
        href="https://twitter.com/knitcodemonkey"
      >
        <img src={jen} alt="" />
        <p>Jen Luker</p>
      </a>
      TBD
    </li>
  </>
)
